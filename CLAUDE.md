...
Each role lives in `roles/<name>/` with standard Ansible structure (`tasks/main.yml`, optional `defaults/` and `vars/`). Active roles in the playbook: network, bluetooth, audio, gpu, firewall, storage, power, bash, libvirt, neovim, nerdfonts, lazygit, dotfiles, stylua, theming, polkit, xdg, sddm, renode.
